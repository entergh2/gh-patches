info_text = [
	...
	"Guarda chi sono i migliori fra i migliori e controlla la tua posizione."
...
script create_online_main_menu \{menu_title_xenon = "ONLINE"
	...
	if NOT isXenon
		if NOT NetSessionFunc \{obj = voice
			...
			CreateScreenElement {
				...
				pos = (640.0, 530.0)
				scale = (0.55, 0.55)
				text = "Chat disattivata sul tuo account PLAYSTATION®Network a causa di restrizioni del filtro contenuti."
				font = text_a4
				rgba = ($online_light_blue)
				z_priority = 6.0
				dims = (1500.0, 120.0)
...
script scroll_motd_ticker \{scroll_time = 8}
	this_id = <id>
	FormatText TextName = msg_text "\\c1MESSAGGIO DEL GIORNO:\\c0 %t" t = ($message_of_the_day)
...
script create_link_text 
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Sei pronto a diventare un Guitar Hero? \\nInvia le tue statistiche alla comunità online:"
		just = [center top]
		internal_just = [center top]
		z_priority = 6.0
		pos = (630.0, 160.0)
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "- Vai sul sito www.guitarhero.com\\n- Crea un nuovo acconto o collegati\\n- Fai clic su 'Link Account'\\n- Immetti il seguente codice VIP"
		just = [center top]
		internal_just = [left top]
		z_priority = 6.0
		pos = (660.0, 240.0)
