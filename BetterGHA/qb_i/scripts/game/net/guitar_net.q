...
net_game_type_info = [
	...
	{
		name = "SFIDA"
		description = "Sezioni di duello con la chitarra solista."
		checksum = p2_faceoff
	}
	{
		name = "BATTAGLIA"
...
script xbox360_live_not_allowed 
	...
	if isXenon
		...
		if GotParam \{age_restricted}
			sign_in_text = "Servizio online disattivato sul tuo account PLAYSTATION®Network a causa di restrizioni del filtro contenuti."
		else
			sign_in_text = "Devi effettuare l'accesso per entrare nella lobby."
		endif
	endif
...
script create_link_unplugged_dialog 
	...
	if IsWinPort
		if (GotParam reason)
			...
			if ($winport_is_in_online_menu_system = 1)
				text = "Sei stato disconnesso. Stai tornando al menu principale..."
			else
...
script net_dl_content_compatabilty_warning \{z = 100
	...
	if IsWinPort
		...
		if isXenon
			text = "È necessario che entrambi i giocatori abbiano gli stessi brani scaricati per poterli utilizzare nelle partite multiplayer su Xbox LIVE."
		else
			text = "È necessario che entrambi i giocatori abbiano gli stessi brani scaricati per poterli utilizzare nelle partite multiplayer sulla PLAYSTATION®Network."
		endif
	endif
	CreateScreenElement {
		type = TextBlockElement
