...
script create_mp_select_difficulty_menu 
	...
	if ($is_network_game = 1)
		...
		displayText parent = sd_container text = "gioc." font = <menu_font> pos = (260.0, 160.0) scale = 1 rgba = [171 189 177 255] noshadow
		GetScreenElementDims id = <id>
		if (<width> > 150)
			SetScreenElementProps id = <id> scale = 1
			fit_text_in_rectangle id = <id> dims = ((150.0, 0.0) + ((0.0, 1.0) * <Height>)) pos = (260.0, 160.0)
		endif
		displayText parent = sd_container text = "gioc." font = <menu_font> pos = (870.0, 160.0) scale = 1 rgba = [171 189 177 255] noshadow
		GetScreenElementDims id = <id>
		if (<width> > 150)
			SetScreenElementProps id = <id> scale = 1
