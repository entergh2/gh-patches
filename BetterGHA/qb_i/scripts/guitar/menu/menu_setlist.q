...
script setlist_footnote \{alpha = 1}
	if NOT ScreenElementExists \{id = setlist_overlay}
		...
		CreateScreenElement {
			...
			scale = (0.7, 0.6)
			text = "*COVER DEI SUCCESSI DEI GRUPPI ELENCATI"
			pos = (0.0, -1.0)
	...
	if (<alpha> = 1)
		...
		if ((<char_array> [(<str_len> - 1)]) = "S")
			FormatText TextName = text "*COVER DEL SUCCESSO DI %a" a = <song_artist>
		else
			FormatText TextName = text "*COVER DEL SUCCESSO DI %a" a = <song_artist>
		endif
			...
			if (<array_size> > 0)
				...
				if (($current_tab = tab_bonus) && (<num_songs_unlocked> = 0))
					CreateScreenElement {
						...
						scale = 1
						just = [center center]
						text = "Non hai ancora acquistato alcun brano."
						rgba = (($g_menu_colors).dk_violet_grey)
						pos = (650.0, 470.0)
						z_priority = 50
						font_spacing = 1
				...
				if (<complete> = 0)
					...
					elseif (<aerosmith_unlocked> = 1)
						...
						if (<num_songs_to_progress> = 1)
							FormatText TextName = completeText "Completa %p brano degli Aerosmith" p = <num_songs_to_progress>
						else
							FormatText TextName = completeText "Completa %p brani degli Aerosmith" p = <num_songs_to_progress>
						endif
					elseif (<boss_unlocked> = 1)
						FormatText \{TextName = completeText
							"Supera il brano del boss"}
					else
						GetGlobalTags <tiername> param = num_intro_songs_to_progress
						if (<num_intro_songs_to_progress> = 1)
							FormatText TextName = completeText "Completa %p brani di apertura" p = <num_intro_songs_to_progress>
						else
							FormatText TextName = completeText "Completa %p brani di apertura degli Aerosmith" p = <num_intro_songs_to_progress>
						endif
					endif
					CreateScreenElement {
						type = TextElement
