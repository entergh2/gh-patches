	...
	if NOT isXenon
		if NOT NetSessionFunc \{obj = voice
			...
			CreateScreenElement {
				...
				pos = (640.0, 530.0)
				scale = (0.55, 0.55)
				text = "Der Chat in Ihrem PLAYSTATION®Network Konto ist aufgrund der Kindersicherung deaktiviert."
				font = text_a4
				rgba = ($online_light_blue)
				z_priority = 6.0
				dims = (1500.0, 120.0)
...
script scroll_motd_ticker \{scroll_time = 8}
	this_id = <id>
	FormatText TextName = msg_text "\\c1NACHRICHT DES TAGES:\\c0 %t" t = ($message_of_the_day)
...
script create_link_text 
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Bist du bereit, ein Gitarrenheld zu sein? \\nVerlinke deine Statistiken mit der Web-Community:"
		just = [center top]
		internal_just = [center top]
		z_priority = 6.0
		pos = (630.0, 160.0)
