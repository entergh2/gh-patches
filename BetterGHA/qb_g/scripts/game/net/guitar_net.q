...
xenon_session_confirm_dialog = "Wenn Du bei dieser Session teilnimmst, gehen alle Fortschritte bei Zielen verloren, an denen Du gerade arbeist. Möchtest Du wirklich fortfahren?"
...
script create_link_unplugged_dialog 
	...
	if IsWinPort
		if (GotParam reason)
			...
			if ($winport_is_in_online_menu_system = 1)
				text = "Ihre Netzwerkverbindung wurde unterbrochen. Zurück zum Hauptmenü."
			else
...
script net_dl_content_compatabilty_warning \{z = 100
	...
	if IsWinPort
		...
		if isXenon
			text = "Beide Spieler müssen die gleichen Heruntergeladenen Songs haben, um sie bei Xbox LIVE Multiplayer-Spielen verwenden zu können."
		else
			text = "Beide Spieler müssen die gleichen Heruntergeladenen Songs haben, um sie bei PLAYSTATION®Network Multiplayer-Spielen verwenden zu können."
		endif
	endif
	CreateScreenElement {
		type = TextBlockElement
