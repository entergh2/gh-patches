...
script rock_back_and_forth_star_meter 
	move_up_and_down = 1
	if ($game_mode = p1_career || $game_mode = p1_quickplay || $game_mode = p2_quickplay || $game_mode = p2_coop)
...
script hud_move_note_scorebar \{player = 1
	...
	if (($game_mode = p2_coop || $game_mode = p2_quickplay) && (<player> = 2))
		return
	endif
...
script hud_show_note_streak_combo \{player = 1
	...
	if NOT ScreenElementExists id = <hud_destroygroup>
		return
	endif
	CreateScreenElement {
		type = ContainerElement
	...
	FormatText TextName = text "%d fache Notenserie!" d = <combo>
