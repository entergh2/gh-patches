...
script setlist_footnote \{alpha = 1}
	if NOT ScreenElementExists \{id = setlist_overlay}
		...
		CreateScreenElement {
			...
			scale = (0.7, 0.6)
			text = "*COVER-VERSIONEN DER HITS DER AUFGELISTETEN BANDS"
			pos = (0.0, -1.0)
	...
	if (<alpha> = 1)
		...
		if ((<char_array> [(<str_len> - 1)]) = "S")
			FormatText TextName = text "*COVERVERSION VON %aS HIT" a = <song_artist>
		else
			FormatText TextName = text "*COVER-VERSION DES HITS VON %a" a = <song_artist>
		endif
			...
			if (<array_size> > 0)
				...
				if IsSongAvailable song_checksum = <song_checksum> song = ($g_gh3_setlist.<tier_checksum>.songs [<song_count>]) for_bonus = <for_bonus>
					...
					if get_song_covered_by song = ($g_gh3_setlist.<tier_checksum>.songs [<song_count>])
						FormatText TextName = song_title "%s (Coverversion)*" s = <song_title>
					endif
					FormatText \{checksumname = textid
						'id_song%i'
						i = $setlist_num_songs
				...
				if (($current_tab = tab_bonus) && (<num_songs_unlocked> = 0))
					CreateScreenElement {
						...
						scale = 1
						just = [center center]
						text = "(Du hast noch keine Songs gekauft)"
						rgba = (($g_menu_colors).dk_violet_grey)
						pos = (650.0, 470.0)
						z_priority = 50
						font_spacing = 1
				...
				if (<complete> = 0)
					...
					elseif (<aerosmith_unlocked> = 1)
						...
						if (<num_songs_to_progress> = 1)
							FormatText TextName = completeText "Meistere %p Aerosmith-Song, um fortzufahren." p = <num_songs_to_progress>
						else
							FormatText TextName = completeText "Meistere %p Aerosmith-Songs, um fortzufahren." p = <num_songs_to_progress>
						endif
					elseif (<boss_unlocked> = 1)
						...
						if (<num_intro_songs_to_progress> = 1)
							FormatText TextName = completeText "Meistere %p Vorgruppen-Song, um fortzufahren." p = <num_intro_songs_to_progress>
						else
							FormatText TextName = completeText "Meistere %p Vorgruppen-Songs, um fortzufahren." p = <num_intro_songs_to_progress>
						endif
					endif
					CreateScreenElement {
						type = TextElement
