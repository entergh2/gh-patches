np_5star_headlines = [
	"%b ROCKT DAS PUBLIKUM MIT IHREM SCHALLANGRIFF"
	"%b ZEIGT ÜBERLEGENE GENIALITÄT"
...
script create_newspaper_menu \{for_practice = 0}
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_coop || $game_mode = p1_quickplay || $game_mode = p2_quickplay || <for_practice> = 1)
		...
		if ($game_mode = p2_battle)
			...
			if ($is_network_game)
				...
				FormatText TextName = who_won_text "%s Dominiert!" s = <winner_text>
			else
				TruncateText text = <winner_text> max_chars = 10
				<winner_text_short> = <trunc_text>
				FormatText TextName = who_won_text "%s Dominiert!" s = <winner_text_short>
			endif
			CreateScreenElement {
				type = TextElement
				parent = newspaper_container
			...
			if (<winner> = "0")
				...
				elseif ((<percent> > 0.25) && (<percent> < 0.75))
					FormatText TextName = who_won_text "%d gewinnt." d = <winner_text>
				else
					FormatText TextName = who_won_text "%d hat knapp gewonnen!" d = <winner_text>
				endif
			endif
			CreateScreenElement {
				type = TextBlockElement
	...
	if NOT IsWinPort
		if (($is_network_game) && (isXenon))
			add_user_control_helper \{text = "SPIELERKARTE"
				button = start
				z = 100}
		endif
	endif
	...
	if (<for_practice> = 1)
		...
		if NOT ($current_song = kingsandqueenscredits)
			if (<show_replay> = 1 && $progression_play_completion_movie = 0)
				displayText id = np_option_1 parent = newspaper_container text = "WIEDERHOLEN" pos = (($g_np_option_props [1].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [1].rot) font = <menu_font> noshadow
				SetScreenElementProps id = <id> font_spacing = 2 space_spacing = 4
