...
script create_encore_confirmation_menu 
	...
	if (<song_struct>.checksum = bossjoe)
		bossText = "SPIELE EINE ZUGABE MIT JOE PERRY!"
		bossdims = (800.0, 0.0)
	endif
	...
	set_focus_color \{color = dark_red3}
	set_unfocus_color \{color = blueish_grey2}
...
script create_aerosmith_confirmation_menu 
	if ($signin_change_happening = 1)
		return
	endif
	ResetScoreUpdateReady
...
script destroy_aerosmith_confirmation_menu 
	restore_start_key_binding
	clean_up_user_control_helpers
	KillSpawnedScript \{name = ec_flashes}
	KillSpawnedScript \{name = ec_raise_fists}
...
script encore_leave 
	if ($g_encore_ready)
		...
		WriteAchievements \{achievement = DRAW_THE_LINE}
...
script ec_raise_fists 
	...
	if ScreenElementExists id = <arm_id>
		...
		if (Random (@ 1 @*80 0 ))
			rand_arm = Random (@ 1 @ 2 @ 3 @ 4 @ 5 @ 6 @ 7 @ 8 )
			SetScreenElementProps id = <arm_id> texture = <arm_tex>
		endif
	endif
	<i> = (<i> + 1)
	repeat 8
