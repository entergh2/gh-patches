...
master_leaderboard_singleplayer_list = [
	...
	{
		leaderboard_id = lb_career_medium
		title = "CARRERA: MEDIA"
		columns = 3
		headers = [
			"LUGAR"
	...
	{
		leaderboard_id = lb_campaign_medium
		title = "GIRA: MEDIA"
		columns = 3
		headers = [
			"LUGAR"
...
master_leaderboard_multiplayer_list = [
	...
	{
		leaderboard_id = lb_faceoff_wins
		title = "DUELO: VICTORIAS"
		columns = 4
		headers = [
			"LUGAR"
			"NOMBRE DE USUARIO"
	...
	{
		leaderboard_id = lb_pro_faceoff_wins
		title = "DUELO PRO: VICTORIAS"
		columns = 4
		headers = [
			"LUGAR"
			"NOMBRE DE USUARIO"
...
script set_leaderboards_presence 
...
script create_leaderboard_group_listing_menu \{menu_id = online_leaderboard_select_menu
	...
	frame_dims = (615.0, 588.0)
	center_pos = ((300.0, 270.0))
	create_UI_frame {
		parent = <cont_id>
		frame_dims = <frame_dims>
		center_pos = <center_pos>
...
script leaderboard_group_add_item {
	...
	SetScreenElementProps {
		id = <id>
		pos = (<bar_pos> + (1.0, 0.0) * <menu_width> - (0.0, 0.0))
	}
...
script leaderboard_group_item_focus 
	...
	if ScreenElementExists id = {<ObjID> child = bar_right}
		SetScreenElementProps {
...
script create_leaderboard_menu 
	...
	switch ($LeaderboardSearchValue)
		...
		case 2
		search_title = "MIS AMIGOS:"
	endswitch
	menu_backdrop_container :SetTags <...>
	create_leaderboard_loading_bar \{read_type = my_score}
	NetSessionFunc {
		obj = stats
		func = get_stats
		params = {
...
script create_leaderboard_loading_bar 
	...
	CreateScreenElement \{type = ContainerElement
		...
		pos = (570.0, 280.0)}
	<cont_id> = <id>
	if GotParam \{read_type}
		<cont_id> :SetTags read_type = <read_type>
	endif
	CreateScreenElement {
		type = TextElement
		parent = <cont_id>
		id = leaderboard_loading_text_id
		font = text_a4
		text = "CARGANDO"
		rgba = ($online_medium_blue)
		just = [left top]
	...
	RunScriptOnScreenElement \{id = leaderboard_loading_text_id
		leaderboard_loading_bar_animate}
...
script create_leaderboard_menu2 
	...
	if ScreenElementExists \{id = net_leaderboards_loading_cont}
		net_leaderboards_loading_cont :GetTags
		printstruct <...>
		if (<read_type> = my_score)
			destroy_leaderboard_loading_bar
			GetArraySize \{leaderboard_data}
			if ($current_leaderboard_group = song)
				my_data = ["sin clasificación" "" "" "sin puntuación"]
			else
				my_data = ["sin clasificación" "" "sin puntuación"]
			endif
			if (<array_size> > 0)
...
script create_leaderboard_menu3 
	...
	if NOT IsWinPort
		if isXenon
			add_user_control_helper \{text = "TARJETA DE JUGADOR"
				button = start
				z = 100}
		endif
	endif
...
script add_leaderboard_rows_to_menu 
	...
	if GotParam \{leaderboard_data}
		...
		if ((<leaderboard_id> = lb_faceoff_wins) || (<leaderboard_id> = lb_pro_faceoff_wins) || (<leaderboard_id> = lb_battle_wins))
		...
		if (<array_size> < 10)
			...
			<leaderboard_entry> :RemoveTags player_xuid
...
script new_leaderboard_menu 
	...
	CreateScreenElement \{type = ContainerElement
		...
		pos = (-10.0, 0.0)}
	anchor_id = <id>
	frame_dims = (860.0, 670.0)
	center_pos = ((650.0, 340.0))
	create_UI_frame {
		parent = <anchor_id>
		frame_dims = <frame_dims>
		center_pos = <center_pos>
	...
	text_scale = (0.8, 1.0)
	FormatText TextName = header_text "\\c8%t\\c0 %s" t = <search_type_title> s = <title>
	CreateScreenElement {
		type = TextElement
		parent = <anchor_id>
		font = fontgrid_title_gh3
	...
	GetScreenElementDims id = <id>
	fit_text_in_rectangle id = <id> dims = ((580.0, 0.0) + (<Height> * (0.0, 1.0))) start_x_scale = 0.8 start_y_scale = 1 only_if_larger_x = 1
	CreateScreenElement {
		type = ContainerElement
		parent = <anchor_id>
		just = [left top]
	...
	if NOT ((<my_data> [0]) = "sin clasificación")
		FormatText TextName = rank "Posición: %d" d = (<my_data> [0])
	else
		rank = (<my_data> [0])
	endif
	CreateScreenElement {
	...
	score = (<my_data> [2])
	if ((($current_leaderboard_id) = lb_faceoff_wins) || (($current_leaderboard_id) = lb_pro_faceoff_wins) || (($current_leaderboard_id) = lb_battle_wins))
	...
	CreateScreenElement {
		...
		event_handlers = [
			...
			{pad_option2 hide_unhide_current_leaderboard params = {hide}}
			{pad_option2 create_leaderboard_filter_dialog}
...
script leaderboard_custom_focus 
	...
	if ScreenElementExists id = {<ObjID> child = bg_bar_left}
		SetScreenElementProps id = {<ObjID> child = bg_bar_left} alpha = 1 preserve_flip
	endif
endscript
...
script leaderboard_custom_unfocus 
	...
	if ScreenElementExists id = {<ObjID> child = bg_bar_left}
		SetScreenElementProps id = {<ObjID> child = bg_bar_left} alpha = 0 preserve_flip
	endif
endscript
...
script leaderboard_scroll \{dir = down}
	if (<dir> = down)
		if ($lb_selection_index = 9)
			if ($lb_offset > 90 && $lb_list_type = friends)
				printf \{"at the end of friends list"}
				return
			endif
...
script request_leaderboard 
	create_leaderboard_loading_bar \{read_type = leaderboard_rows}
	NetSessionFunc {
		obj = stats
		func = get_stats
		params = {
