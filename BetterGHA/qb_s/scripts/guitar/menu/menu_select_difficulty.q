...
script create_mp_select_difficulty_menu 
	...
	if ($is_network_game = 1)
		...
		displayText parent = sd_container text = "JUG." font = <menu_font> pos = (260.0, 160.0) scale = 1 rgba = [171 189 177 255] noshadow
		GetScreenElementDims id = <id>
		if (<width> > 150)
			SetScreenElementProps id = <id> scale = 1
			fit_text_in_rectangle id = <id> dims = ((150.0, 0.0) + ((0.0, 1.0) * <Height>)) pos = (260.0, 160.0)
		endif
		displayText parent = sd_container text = "JUG." font = <menu_font> pos = (870.0, 160.0) scale = 1 rgba = [171 189 177 255] noshadow
		GetScreenElementDims id = <id>
		if (<width> > 150)
			SetScreenElementProps id = <id> scale = 1
...
script sd_create_score_balancing_notice 
	...
	if ((<ONE> = 0 && <two> = 1) || (<ONE> = 1 && <two> = 0))
		<average_diff> = "MEDIA-FÁCIL"
	elseif ((<ONE> = 0 && <two> = 2) || (<ONE> = 2 && <two> = 0))
		<average_diff> = "MEDIO"
	elseif ((<ONE> = 0 && <two> = 3) || (<ONE> = 3 && <two> = 0))
		<average_diff> = "MEDIA-ALTA"
	elseif ((<ONE> = 1 && <two> = 2) || (<ONE> = 2 && <two> = 1))
		<average_diff> = "MEDIA-ALTA"
	elseif ((<ONE> = 1 && <two> = 3) || (<ONE> = 3 && <two> = 1))
		<average_diff> = "DIFÍCIL"
	elseif ((<ONE> = 2 && <two> = 3) || (<ONE> = 3 && <two> = 2))
		<average_diff> = "EXPERTO-DIFÍCIL"
	endif
	FormatText TextName = score_balancing_text "LA PUNTUACIÓN SE BASA EN UNA DIFICULTAD %a" a = <average_diff>
	if ScreenElementExists \{id = score_balancing_container}
		DestroyScreenElement \{id = score_balancing_container}
	endif
	CreateScreenElement \{type = ContainerElement
