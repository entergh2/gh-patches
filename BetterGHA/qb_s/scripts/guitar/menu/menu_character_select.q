...
script create_net_game_info_panel \{parent = user_control_container
	...
	printstruct <...>
	FormatText TextName = mode_text "MODO: %d" d = <ui_string>
	CreateScreenElement {
		type = TextElement
		parent = <cont_id>
		font = fontgrid_title_gh3
	...
	if ($match_type = ranked)
		...
		printstruct <...>
		FormatText TextName = diff_text "DIFICULTAD: %d" d = <ui_string>
		CreateScreenElement {
			type = TextElement
			parent = <cont_id>
			font = fontgrid_title_gh3
	...
	text_pos_rt = (<text_pos_rt> + (0.0, 1.0) * <Height> + (0.0, 1.0) * <text_space>)
	get_number_of_songs
	FormatText TextName = num_songs_text "TEMAS: %d" d = <num_songs>
	CreateScreenElement {
		type = TextElement
		parent = <cont_id>
		font = fontgrid_title_gh3
