np_5star_headlines = [
	"EL PÚBLICO ENLOQUECE CON LA DESCARGA DE VATIOS DE %b"
	"%b NO TIENE RIVAL SOBRE EL ESCENARIO"
	"10 DE CADA 10 ROCKEROS ADORAN A %b"
	"¡ENCERRAD A VUESTRAS HIJAS EN CASA! HA LLEGADO %b."
]
		...
		if ($game_mode = p2_battle)
			...
			if (<winner> = "0")
				...
				elseif ((<percent> > 0.25) && (<percent> < 0.75))
					FormatText TextName = who_won_text "%d gana." d = <winner_text>
				else
					FormatText TextName = who_won_text "¡%d logra la victoria por poco!" d = <winner_text>
				endif
			endif
			CreateScreenElement {
				type = TextBlockElement
...
script np_create_options_menu \{pos = (600.0, 300.0)
	...
	if (<for_practice> = 1)
		...
		displayText id = np_option_1 parent = newspaper_container text = "REINICIAR" pos = (($g_np_option_props [5].pos) + <menu_offset>) scale = (0.8, 0.7) rot = ($g_np_option_props [5].rot) font = <menu_font> noshadow
