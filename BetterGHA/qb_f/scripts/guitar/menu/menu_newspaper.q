np_5star_headlines = [
	...
	"%b A DONNÉ UN SHOW GRANDIOSE"
	"10 ROCKEURS SUR 10 SONT FANS DE %b"
	"ENFERME TA SOEUR CHEZ TOI ! %b EST DANS LA PLACE"
]
	...
	if NOT ($game_mode = p1_career || $game_mode = p2_coop || $game_mode = p1_quickplay || $game_mode = p2_quickplay || <for_practice> = 1)
		...
		if ($game_mode = p2_battle)
			...
			if ($is_network_game)
				...
				FormatText TextName = who_won_text "%s  domine !" s = <winner_text>
			else
				TruncateText text = <winner_text> max_chars = 10
				<winner_text_short> = <trunc_text>
				FormatText TextName = who_won_text "%s  domine !" s = <winner_text_short>
			endif
			CreateScreenElement {
				type = TextElement
				parent = newspaper_container
			...
			if (<winner> = "0")
				...
				elseif ((<percent> > 0.25) && (<percent> < 0.75))
					FormatText TextName = who_won_text "%d gagne !" d = <winner_text>
				else
					FormatText TextName = who_won_text "Victoire de justesse de %d !" d = <winner_text>
				endif
			endif
			CreateScreenElement {
				type = TextBlockElement
