...
script setlist_footnote \{alpha = 1}
	if NOT ScreenElementExists \{id = setlist_overlay}
		...
		CreateScreenElement {
			...
			scale = (0.7, 0.6)
			text = "*REPRISE DES TUBES DES GROUPES LISTÉS"
			pos = (0.0, -1.0)
	...
	if (<alpha> = 1)
		...
		if ((<char_array> [(<str_len> - 1)]) = "S")
			FormatText TextName = text "*REPRISE DU TUBE DE %a" a = <song_artist>
		else
			FormatText TextName = text "*REPRISE DU TUBE DE %a" a = <song_artist>
		endif
			...
			if (<array_size> > 0)
				...
				if IsSongAvailable song_checksum = <song_checksum> song = ($g_gh3_setlist.<tier_checksum>.songs [<song_count>]) for_bonus = <for_bonus>
					...
					if get_song_covered_by song = ($g_gh3_setlist.<tier_checksum>.songs [<song_count>])
						FormatText TextName = song_title "%s (reprise)*" s = <song_title>
					endif
					FormatText \{checksumname = textid
						'id_song%i'
						i = $setlist_num_songs
				...
				if (($current_tab = tab_bonus) && (<num_songs_unlocked> = 0))
					CreateScreenElement {
						...
						scale = 1
						just = [center center]
						text = "(tu n'as acheté aucune chanson)"
						rgba = (($g_menu_colors).dk_violet_grey)
						pos = (650.0, 470.0)
						z_priority = 50
						font_spacing = 1
				...
				if (<complete> = 0)
					...
					elseif (<aerosmith_unlocked> = 1)
						...
						if (<num_songs_to_progress> = 1)
							FormatText TextName = completeText "Joue %p chanson d'Aerosmith." p = <num_songs_to_progress>
						else
							FormatText TextName = completeText "Joue %p chansons d'Aerosmith." p = <num_songs_to_progress>
						endif
					elseif (<boss_unlocked> = 1)
						FormatText \{TextName = completeText
							"Réussis la chanson du boss."}
					else
						GetGlobalTags <tiername> param = num_intro_songs_to_progress
						if (<num_intro_songs_to_progress> = 1)
							FormatText TextName = completeText "Joue %p chanson pour continuer." p = <num_intro_songs_to_progress>
						else
							FormatText TextName = completeText "Joue %p chansons pour continuer." p = <num_intro_songs_to_progress>
						endif
					endif
					CreateScreenElement {
						type = TextElement
...
script setlist_show_helperbar \{text_option1 = "LE COFFRE-FORT"
		button_option1 = "\\b7"
		spacing = 16}
	...
	<i> = (<i> + 1)
	repeat 4
	tabs_text = ["SÉLECTION" "Le Coffre-fort"]
