...
script create_online_main_menu \{menu_title_xenon = "EN LIGNE"
		menu_title_ps3 = "EN LIGNE"
	...
	if NOT isXenon
		if NOT NetSessionFunc \{obj = voice
			...
			CreateScreenElement {
				...
				pos = (640.0, 530.0)
				scale = (0.55, 0.55)
				text = "Le chat est désactivé sur votre compte PLAYSTATION®Network en raison des restrictions de contrôle parental."
				font = text_a4
				rgba = ($online_light_blue)
				z_priority = 6.0
				dims = (1500.0, 120.0)
...
script scroll_motd_ticker \{scroll_time = 8}
	this_id = <id>
	FormatText TextName = msg_text "\\c1MESSAGE DU JOUR :\\c0 %t" t = ($message_of_the_day)
...
script create_link_text 
	...
	CreateScreenElement {
		...
		scale = (0.75, 0.75)
		rgba = ($online_light_blue)
		text = "Prêt à devenir un Guitar Hero ? \\nTu peux voir tes stats sur le site Internet :"
		just = [center top]
		internal_just = [center top]
		z_priority = 6.0
		pos = (630.0, 160.0)
