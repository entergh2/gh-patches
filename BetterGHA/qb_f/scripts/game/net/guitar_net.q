...
net_game_type_info = [
	...
	{
		name = "CHOC DE GUITARES"
		description = "mode Choc."
		checksum = p2_battle
	}
	{
		name = "COOP"
...
script xbox360_live_not_allowed 
	...
	if isXenon
		...
		if GotParam \{age_restricted}
			sign_in_text = "Le service en ligne est désactivé sur votre compte PLAYSTATION®Network en raison des restrictions de contrôle parental."
		else
			sign_in_text = "Vous devez être connecté pour accéder à l'accueil."
		endif
	endif
...
script create_link_unplugged_dialog 
	...
	if IsWinPort
		if (GotParam reason)
			...
			if ($winport_is_in_online_menu_system = 1)
				text = "La connexion réseau a été interrompue. Retour au menu principal."
			else
...
script net_dl_content_compatabilty_warning \{z = 100
	...
	if IsWinPort
		...
		if isXenon
			text = "Les deux joueurs doivent avoir les mêmes Chansons téléchargées pour pouvoir les utiliser en partie multijoueur sur Xbox LIVE."
		else
			text = "Les deux joueurs doivent avoir les mêmes Chansons téléchargées pour pouvoir les utiliser en partie multijoueur PLAYSTATION®Network."
		endif
	endif
	CreateScreenElement {
		type = TextBlockElement
